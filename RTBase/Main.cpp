...
static std::string resolveScenePath(const std::string& sceneName)
{
	std::ifstream direct(sceneName + "/scene.json");
	if (direct.good())
	{
		return sceneName;
	}
	...
	std::string rtBasePath = "RTBase/" + sceneName;
	std::ifstream underRTBase(rtBasePath + "/scene.json");
	if (underRTBase.good())
	{
		return rtBasePath;
	}
...
int main(int argc, char *argv[])
{
	...
	sceneName = resolveScenePath(sceneName);
	Scene* scene = loadScene(sceneName);
	if (scene == nullptr)
	{
		std::cerr << "Error: Failed to load scene '" << sceneName << "'" << std::endl;
		return 1;
	}
